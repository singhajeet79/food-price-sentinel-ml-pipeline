...
def load_recent_scores(window_hours: int) -> tuple[np.ndarray, int]:
    ...
    with get_session() as session:
        all_rows = (
            session.query(AnomalyLog).filter(AnomalyLog.detected_at >= cutoff).all()
        )
...
def compute_drift(
    ...
    if "anomaly_rate=" in notes:
        try:
            baseline_rate = float(
                notes.split("anomaly_rate=")[1].split(" |")[0].strip()
            )
...
def write_drift_to_valkey(drift_result: dict) -> None:
    ...
    try:
        ...
        log.info(
            "Drift result written to Valkey: key=%s signal=%s",
            key,
            drift_result["signal"],
        )
...
def run(args: argparse.Namespace) -> int:
    ...
    if signal == "STABLE":
        ...
        log.warning(
            "Recent: p50=%.4f rate=%.4f | Baseline: p50=%.4f rate=%.4f",
            result["recent_p50"],
            result["recent_rate"],
            result["baseline_p50"],
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Detect drift in Food Price Sentinel model"
    )
    parser.add_argument(
        "--window-hours",
        type=int,
